...
| `smpte-analyzer/src/ltcDecoder.js` | DSP: `LtcDecoder` (biphase decode with rolling state; `pendingFrames` queue for continuity tracking; `recentDecodeTimes` for dropout rate), `MultiRateDecoder` (parallel-rate auto-detection, `carrierRate()` (timing-only), `cadence()` (delegates to CadenceDetector), `carrierCadenceMismatch()`, `driftPpm()`, `dropoutPct()`, `medianFrameSpan()`, `detectedRateKey()` (combined view, retained for compat)), `tcToFrameNumber()`, `tcString()`, `parseFrame`, rate key helpers |
